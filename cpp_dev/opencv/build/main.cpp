...
//Extrinsics
const Mat R, R1, R2, P1, P2, Q;
//Intrinsics
const Mat M1, M2, D1, D2;
//Remap
const Mat rmap[2][2];
...
int readAndSetStereoCalibValues(string intrinsic, string extrinsic,int cameraside, Size imageSize)
{
        //READ VALUES FROM GENERATED YAMLS
        FileStorage fs;
        try{fs.open(extrinsic, FileStorage::READ);}
        catch(...)
        {cout<<"Error n firle read."<<endl; return -1;}
        /*
        fs["R"] >> R;
        fs["R1"] >> R1; //left
        cout<<"R1 "<<R1<<endl;
        ...
        {
                cout<<"camera side input value incorrect. Aborting"; return -1;
        }
        */
        return 0;
}
void applySteroCalib(Mat *img)
...
int main()
{
        ...
        readAndSetStereoCalibValues("intrinsics.yml", "extrinsics.yml", 1, Size(frame_width,frame_height));
        //Setup Camera
